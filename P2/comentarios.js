...
*/
const palabras_prohibidas = ["gilipollas", "retrasado", "puta", "inutil", "tonta", "tonto", "zorra", "pito", "capullo", "polla"];
...
function corregirComentarioTiempoReal() //Problema, último caracter no lo coge debido al onkeydown
{
  comentario_final = document.getElementById('opinion').value;

  for (palabra of palabras_prohibidas)
      if (comentario_final.match(palabra))
        document.getElementById('opinion').value = comentario_final.replace(palabra, "*".repeat(palabra.length)); //Que sustituya por tantos ** como letras tenga la palabra
}
...
function verificarComentario()
{
  ...
  else  //Si no hay errores, poner el comentario
  {
      document.getElementById('error').innerHTML = '';
      ponerComentario(document.getElementById('nombre').value, document.getElementById('opinion').value);
  }
...
function ponerComentario(autor, texto)
{
  ...
  /* Para hacer un único comentario
  document.getElementById("autor_comentario").innerHTML = autor;
  document.getElementById("texto_comentario").innerHTML = corregirComentario(texto);
  document.getElementById("hora_comentario").innerHTML = fecha.getHours()+":"+fecha.getMinutes();
  document.getElementById("fecha_comentario").innerHTML = fecha.getDate() + "/" + (fecha.getMonth()+1) + "/" + (fecha.getFullYear());

  comentario.style.display = "block"; //Mostrar comentario como bloque
